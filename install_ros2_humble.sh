...
sudo apt update && sudo apt upgrade -y && sudo apt install -y ros-$name_ros_version-desktop ros-$name_ros_version-cartographer* ros-$name_ros_version-camera-info-manager ros-$name_ros_version-image-publisher ros-$name_ros_version-magic-enum ros-$name_ros_version-robot-localization libuvc-dev ros-$name_ros_version-nav2-* ros-$name_ros_version-octomap-* ros-$name_ros_version-slider-publisher ros-$name_ros_version-xacro libqt5serialport5-dev libgoogle-glog-dev nlohmann-json3-dev libpcap-dev libasio-dev

echo "[Environment setup]"
source /opt/ros/$name_ros_version/setup.sh
sudo apt install -y python3-argcomplete python3-colcon-common-extensions python3-vcstool git python3-pip sshpass python3-wxgtk4.0
pip3 install psutil pygame
